...
class Program
{
    ...
    private static extern int run_parallel_batch_job(string inputPath, string outputPath);

    [DllImport("rust_csv.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern int run_parallel_batch_job_for_huge_file(string inputPath, string outputPath);
